#!/usr/bin/env python3
"""ENCORI task: query mRNA to miRNA interactions."""
...
if __package__ in {None, ""}:
    ...
    from webpages.rnasysu_com.encori.common.io import safe_filename, write_csv_rows, write_json, write_text
else:
    from protocol_gate import ProtocolGateError, infer_input_type, validate_protocol_ticket
...
DEFAULT_DOWNLOAD_ENDPOINT = "https://rnasysu.com/encori/moduleDownload.php"
DEFAULT_HOMEPAGE = "https://rnasysu.com/encori/agoClipRNA.php?source=mRNA"
DEFAULT_JOB_ROOT = ROOT / "outputs" / "tasks"
DEFAULT_JOB_TAG = "mRNA_miRNA_ENCORI"
...
class EncoriMrnaError(RemoteServiceError):
    """Raised when the ENCORI mRNA->miRNA service returns an invalid response."""


class EncoriMrnaClient:
    """Client for ENCORI mRNA-target text downloads."""
    ...
    def build_request_url(
        ...
        *,
        ...
        return f"{self.download_endpoint}?{parse.urlencode(params)}"
    ...
    def fetch_response_text(
        ...
        *,
        ...
        try:
            ...
            raise EncoriMrnaError(str(exc)) from exc


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch query ENCORI and export miRNA hits for human gene symbols.",
    )
    parser.add_argument(
        "--gene",
        action="append",
...
def parse_response_table(payload: str) -> tuple[list[str], list[dict[str, str]], str | None]:
    ...
    if not lines:
        raise EncoriMrnaError("ENCORI response does not contain a tab-delimited table.")

    reader = csv.reader(StringIO("\n".join(lines)), delimiter="\t")
    table = [[cell.strip() for cell in row] for row in reader]
    header = table[0]
    if not header:
        raise EncoriMrnaError("ENCORI response header is empty.")
...
def flatten_result_rows(
    ...
    for row in rows:
        merged = {"query_gene_symbol": query_gene_symbol, "query_mirna_count": 0}
...
def build_summary_entry(response_header: list[str], rows: list[dict[str, str]]) -> dict[str, Any]:
    unique_mirnas = sorted(
        {
            str(row.get("miRNAname") or "").strip()
            for row in rows
            if str(row.get("miRNAname") or "").strip()
        }
    )
    return {
...
def resolve_job_dir(args: argparse.Namespace, genes: list[str]) -> Path:
    if args.job_dir:
        return args.job_dir
    return DEFAULT_JOB_ROOT / f"{DEFAULT_JOB_TAG}_{args.assembly}_{derive_job_name(args, genes)}"
...
def build_output_layout(
    *,
    ...
    if args.job_dir and args.output_prefix:
        raise EncoriMrnaError("Use either --job-dir or --output-prefix, not both.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    client = EncoriMrnaClient(timeout=args.timeout)
    ...
    fieldnames = ["query_gene_symbol", "query_mirna_count"]
    ...
    for index, gene in enumerate(genes, start=1):
        ...
        if isinstance(raw_dir, Path):
            raw_path = raw_dir / f"{index:03d}_{safe_filename(gene)}.tsv"
        ...
        flattened_rows = flatten_result_rows(gene, response_header, rows)
        summary_entries[gene] = build_summary_entry(response_header, rows)
        annotate_rows_with_query_count(flattened_rows, summary_entries[gene]["mirna_count"])
        all_rows.extend(flattened_rows)

        print(
            f"  {len(flattened_rows)} detailed rows, {summary_entries[gene]['mirna_count']} unique miRNAs",
            file=sys.stderr,
        )
