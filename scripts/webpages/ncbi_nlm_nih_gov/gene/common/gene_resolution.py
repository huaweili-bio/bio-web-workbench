...
class NcbiGeneError(RemoteServiceError):
...
class NcbiGeneClient:
    ...
    def fetch_product_report(self, gene_symbol: str, taxon: str) -> tuple[str, dict[str, Any]]:
        ...
        try:
            ...
            raise NcbiGeneError(str(exc)) from exc
...
def _accession_prefix_rank(accession_version: str) -> int:
...
def rank_transcript_row(row: dict[str, Any]) -> tuple[Any, ...]:
    accession_version = str(row.get("transcript_accession_version") or "")
    select_category = str(row.get("transcript_select_category") or "")
    transcript_length = int(row.get("transcript_length") or 0)
    return (-_select_category_rank(select_category), -_accession_prefix_rank(accession_version), -transcript_length, accession_version)
...
def flatten_product_report(query_gene_symbol: str, product: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for transcript in product.get("transcripts", []):
        ...
        cds_range = (cds.get("range") or [{}])[0]
        genomic_location = (transcript.get("genomic_locations") or [{}])[0]
        genomic_range = genomic_location.get("genomic_range") or {}
        protein = transcript.get("protein") or {}
        accession_version = _stringify(transcript.get("accession_version"))
        rows.append(
            {
                ...
                "transcript_rank": 0,
                "transcript_is_recommended": 0,
                "transcript_accession_version": accession_version,
                "transcript_accession": accession_version.split(".", 1)[0] if accession_version else "",
                ...
                "ensembl_transcript": _stringify(transcript.get("ensembl_transcript")),
                "protein_accession_version": _stringify(protein.get("accession_version")),
                "protein_name": _stringify(protein.get("name")),
                "cds_begin": _stringify(cds_range.get("begin")),
                "cds_end": _stringify(cds_range.get("end")),
                "cds_orientation": _stringify(cds_range.get("orientation")),
                "genomic_accession_version": _stringify(genomic_location.get("genomic_accession_version")),
                "genomic_begin": _stringify(genomic_range.get("begin")),
                "genomic_end": _stringify(genomic_range.get("end")),
                "genomic_orientation": _stringify(genomic_range.get("orientation")),
                "exon_count": len(genomic_location.get("exons") or []),
            }
        )
    rows.sort(key=rank_transcript_row)
    for index, row in enumerate(rows, start=1):
        row["transcript_rank"] = index
        row["transcript_is_recommended"] = 1 if index == 1 else 0
    return rows
...
def parse_product_report(payload: dict[str, Any], query_gene_symbol: str) -> tuple[dict[str, Any] | None, list[dict[str, Any]]]:
    ...
    if not isinstance(report, dict):
        raise NcbiGeneError(f"NCBI product report for {query_gene_symbol} is malformed.")
    product = report.get("product")
    if not isinstance(product, dict):
        raise NcbiGeneError(f"NCBI product report for {query_gene_symbol} does not contain product details.")
...
def build_gene_summary_entry(product: dict[str, Any] | None, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "recommended_transcript_accession_version": _stringify(recommended_row.get("transcript_accession_version")),
        "recommended_transcript_accession": _stringify(recommended_row.get("transcript_accession")),
        "recommended_select_category": _stringify(recommended_row.get("transcript_select_category")),
        "transcript_accessions": [_stringify(row.get("transcript_accession_version")) for row in rows if _stringify(row.get("transcript_accession_version"))],
    }


def annotate_rows_with_query_count(rows: list[dict[str, Any]], query_transcript_count: int) -> None:
    for row in rows:
        row["query_transcript_count"] = query_transcript_count
