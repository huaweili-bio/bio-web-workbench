"""Machine-readable metadata for the TargetScanHuman 8.0 webpage package."""

from __future__ import annotations

from pathlib import Path


PAGE_DIR = Path(__file__).resolve().parent
SCRIPTS_DIR = PAGE_DIR.parents[2]
ROOT = SCRIPTS_DIR.parent
TASK_SCRIPT = PAGE_DIR / "tasks" / "mrna_to_mirna.py"
COMMANDS_DOC = PAGE_DIR / "COMMANDS.md"
README_DOC = PAGE_DIR / "README.md"
DEFAULT_JOB_DIR = ROOT / "outputs" / "tasks" / "mRNA_miRNA_TargetScan_default"
SMOKE_JOB_DIR = ROOT / "outputs" / "tasks" / "mRNA_miRNA_TargetScan_smoke_APP"
DEFAULT_CACHE_DIR = ROOT / "outputs" / "cache" / "targetscan_org__vert_80"

PAGE_METADATA = {
    "page_key": "targetscan_org.vert_80",
    "homepage": "https://www.targetscan.org/vert_80/",
    "page_type": "webpage",
    "interaction_mode": "download_table",
    "status": "active",
    "code_root": str(PAGE_DIR),
    "readme": str(README_DOC),
    "commands_doc": str(COMMANDS_DOC),
}

TASKS = {
    "mrna_to_mirna": {
        "task_key": "mrna_to_mirna",
        "bio_category": "mRNA",
        "bio_goal": "mRNA biomarker -> mature miRNA predicted target details",
        "description": "Filter TargetScanHuman 8.0 predicted target detail tables by human biomarker gene symbols, with optional local-mode score archives.",
        "entrypoint_script": str(TASK_SCRIPT),
        "preferred_output_mode": "job_dir",
        "default_job_dir": str(DEFAULT_JOB_DIR),
        "master_file_name": "targetscanhuman_result.csv",
        "master_file_mode": "direct_generated_master_file",
        "master_file_is_concatenated": False,
        "master_file_required_columns": ["query_gene_symbol", "query_mirna_count", "gene_symbol", "mirna", "site_conservation"],
        "temp_dir_name": "temp",
        "protocol_check_required": True,
        "batch_input_requires_subagent": True,
        "test_output_cleanup_required": True,
        "test_output_cleanup_scope": (
            "Delete smoke/test job directories after verification unless the user explicitly asks to keep them."
        ),
        "help_command": f'python "{TASK_SCRIPT}" --help',
        "smoke_command": (
            f'python "{TASK_SCRIPT}" --gene APP '
            f'--job-dir "{SMOKE_JOB_DIR}"'
        ),
        "task_command_template": (
            f'python "{TASK_SCRIPT}" --input "F:\\bio-script\\data\\biomarker_genes.txt" '
            f'--job-dir "{DEFAULT_JOB_DIR}"'
        ),
        "long_running": False,
        "default_cache_dir": str(DEFAULT_CACHE_DIR),
        "result_reporting_template": [
            "script used",
            "input file",
            "master file mode",
            "job directory",
            "master file path",
            "errors file presence",
            "gene hit count",
            "final row count",
        ],
    }
}

DELEGATION_NOTES = {
    "background": (
        "This page is backed by official downloadable TargetScanHuman 8.0 tables. "
        "The implemented task filters mature-miRNA predicted target detail tables for human gene symbols. "
        "The two large score archives can also be provided locally via local_data mode."
    ),
    "long_task_policy": {
        "delegate_to_subagent": True,
        "batch_input_requires_subagent": True,
        "main_thread_waits_to_completion": False,
        "main_thread_default_reply": [
            "delegated",
            "subagent name",
            "execution command",
            "output path",
        ],
        "query_results_later": True,
        "stop_on_user_request": True,
    },
    "workflow": [
        "Read README.md, COMMANDS.md, and manifest.py before executing.",
        "Run the help command or smoke command first.",
        "Only after a successful smoke run should you attempt a larger task.",
        "Delete smoke/test outputs after verification unless the user explicitly asks to keep them.",
        "Expect the first run to initialize a webpage-specific cache by downloading the official ZIP file.",
    ],
    "must_include": [
        "background information",
        "task goal",
        "recommended command",
        "smoke/test command",
        "forbidden long-running behavior",
    ],
    "do_not": [
        "Do not introduce cross-webpage common helpers.",
        "Do not run long batch jobs by default.",
        "Do not keep smoke/test output directories after validation unless the user explicitly asks to keep them.",
        "Do not discover biomarkers from literature in this webpage package.",
    ],
}
