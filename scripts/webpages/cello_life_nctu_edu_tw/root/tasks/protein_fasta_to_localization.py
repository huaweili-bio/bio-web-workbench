#!/usr/bin/env python3
"""CELLO task: predict localization from protein FASTA."""
...
if __package__ in {None, ""}:
    ...
    from webpages.cello_life_nctu_edu_tw.root.common.core import CelloError, copy_input_artifacts, parse_fasta_records, parse_prediction_html, submit_query, write_csv_rows, write_json
else:
    from protocol_gate import ProtocolGateError, infer_input_type, validate_protocol_ticket
    from ..common.core import CelloError, copy_input_artifacts, parse_fasta_records, parse_prediction_html, submit_query, write_csv_rows, write_json
...
DEFAULT_JOB_TAG = "ProteinLoc_CELLO"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Submit protein or coding nucleotide FASTA records to CELLO and parse protein localization predictions.")
    parser.add_argument("--input", type=Path, required=True, help="Input FASTA file. Protein by default; use --seqtype dna for coding nucleotide input.")
    parser.add_argument("--job-dir", type=Path, help="Preferred output directory.")
    parser.add_argument("--species", default="eu", help="CELLO species code. Default: eu.")
    parser.add_argument("--seqtype", default="prot", help="CELLO sequence type. Use prot for protein FASTA or dna for coding nucleotide input. Default: prot.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        protocol_payload = validate_protocol_ticket(
            ...
            page_key=PAGE_KEY,
            task_key=TASK_KEY,
            input_type=infer_input_type(query_count=len(fasta_records), input_file=args.input),
            job_dir=job_dir,
        )
    ...
    for record in fasta_records:
        try:
            payload = submit_query(fasta_record=record, species=args.species, seqtype=args.seqtype, timeout=args.timeout)
    ...
    if result_status == "failed":
        print("CELLO failed for all sequences.", file=sys.stderr)
        return 1
    return 0
